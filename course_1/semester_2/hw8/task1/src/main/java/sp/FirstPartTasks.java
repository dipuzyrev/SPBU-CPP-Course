...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums
                .flatMap((a) -> a.getTracks().stream().map((track -> track.getName())))
                .sorted().collect(Collectors.toList());
    }

    // Список альбомов, в которых есть хотя бы один трек с рейтингом более 95, отсортированный по названию
    public static List<Album> sortedFavorites(Stream<Album> s) {
        return s.filter(album -> album.getTracks().stream().filter(track -> track.getRating() > 95).count() > 0)
                .sorted(Comparator.comparing(Album::getName, String::compareTo))
    ...
    public static Map<Artist, List<String>> groupByArtistMapName(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getArtist, Collectors.
                mapping((album) -> album.getName(), Collectors.toList())));
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.map(album -> new Pair<>(album,
                    album.getTracks().stream().mapToInt(track -> track.getRating()).average().orElse(0)))
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream
                .reduce(1, (previous, current) -> (previous = (previous * current) % modulo));
    }
